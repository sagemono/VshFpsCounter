...
bool AppendFile(const std::string& filePath, const void* data, size_t size)
{
    int fd;
    if (cellFsOpen(filePath.c_str(), CELL_FS_O_WRONLY | CELL_FS_O_CREAT | CELL_FS_O_APPEND, &fd, nullptr, 0) == CELL_FS_SUCCEEDED)
    {
        cellFsWrite(fd, data, size, nullptr);
        cellFsClose(fd);
        return true;
