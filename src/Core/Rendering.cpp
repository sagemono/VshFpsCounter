...
void Render::Text(const std::wstring& text, vsh::vec2 position, float height, Align horizontalAlign, Align verticalAlign, vsh::vec4 color, float angle)
{
    ...
    // shadow pass: dark text at slight offset for readability over any background
    {
        vsh::paf::PhText* shadow = GetText(m_CurrentText);
        if (!shadow || !shadow->IsAttached())
            return;

        vsh::vec2 shadowPos = position;
        shadowPos.x += 0.7f;
        shadowPos.y -= 0.7f;

        shadow->SetText(text)
            .SetTextHeight(height * 0.8)
            .SetPosition(shadowPos)
            .SetColor(vsh::vec4(0, 0, 0, 0.9f))
            ...
            .SetStyle(vsh::paf::PhWidget::TextShadow, false)
...
void Render::CreateWidgets()
{
    ...
    vsh::srand(clock() ^ time(NULL) ^ sys_process_getpid());
