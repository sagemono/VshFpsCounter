...
private:
    ...
    // perf stats (VSHfallback, used in XMB mode)
    void UpdatePerformance(float frameTimeSeconds);
    bool AnyPerfEnabledForCurrentMode() const;

    // Game side sensor (reads from LV2 shared memnory)
...
private:
    ...
    // Performance samples/metrics (VSH side fallback)
    struct FrameSample
    {
        uint64_t timeMs;     // GetTimeNow()
        float frameTimeMs;   // frametime
    };
    ...
    // display / derived (written by either sensor or vsh computation)
