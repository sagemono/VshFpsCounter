...
struct Accessor
{
    ...
    virtual ~Accessor() {}
    virtual Accessor operator[](int index) {
        if (!node) return Accessor(nullptr, owner);
        auto fn = getOp(node->type).op_sq_int;
        return fn ? fn(this, index) : Accessor(nullptr, owner);
    }
    virtual Accessor operator[](const std::string& key) {
        if (!node) return Accessor(nullptr, owner);
        auto fn = getOp(node->type).op_sq_str;
        return fn ? fn(this, key) : Accessor(nullptr, owner);
    }
    virtual Accessor operator[](const char* key) {
        if (!node) return Accessor(nullptr, owner);
        auto fn = getOp(node->type).op_sq_chp;
        return fn ? fn(this, key) : Accessor(nullptr, owner);
    }
    virtual Accessor nodeWith(const std::string& name, const std::string& key) {
        if (!node) return Accessor(nullptr, owner);
        return getOp(node->type).nodeWith(this, name, key);
    }
    virtual Accessor tryNodeWith(const std::string& name, const std::string& key) {
        if (!node) return Accessor(nullptr, owner);
        return getOp(node->type).tryNodeWith(this, name, key);
    }

    virtual std::string str() {
        if (!node) return std::string();
        auto fn = getOp(node->type).str;
        return fn ? fn(this) : std::string();
    }
    virtual double dbl_debug() {
        if (!node) return 0.0;
        auto fn = getOp(node->type).dbl;
        return fn ? fn(this) : 0.0;
    }
    virtual double dbl()
    {
        if (!node) return 0.0;
        auto fn = getOp(node->type).str;
        if (!fn) return 0.0;
        const std::string& value = fn(this);
...
public:
    Accessor root()
    {
        return Accessor(m_root, this); // m_root may be null if parse failed
    }
...
inline std::string Accessor::str_str(Accessor* that) {
    if (!that->node) return std::string();
    auto& s = ((DataNode*)that->node)->str;
